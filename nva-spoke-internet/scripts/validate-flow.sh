...
# Network Watcher probes are informational / diagnostic only.
#
# Usage:
#   ./scripts/validate-flow.sh
#   RESOURCE_GROUP=my-rg HUB=my-hub ./scripts/validate-flow.sh
#
...
# =============================================================================
# Phase 3 -- Data-plane: curl from spoke VMs (proves SNAT egress via NVA)
...
log "  Expected: spoke VMs return this IP proving SNAT through NVA/Public LB"
...
if [[ -n "$PUBLIC_LB_PIP" && "$CURL1" == "$PUBLIC_LB_PIP" ]]; then
  check_pass "vm-spoke1 egress IP = $PUBLIC_LB_PIP (Public LB PIP) -- SNAT through NVA confirmed"
...
if [[ -n "$PUBLIC_LB_PIP" && "$CURL2" == "$PUBLIC_LB_PIP" ]]; then
  check_pass "vm-spoke2 egress IP = $PUBLIC_LB_PIP (Public LB PIP) -- SNAT through NVA confirmed"
...
# (iptables MASQUERADE counters, conntrack, concurrent tcpdump)
# =============================================================================
log ""
log "=== Phase 4: NVA forwarding evidence ==="

for NVA_NAME in nva-dmz-0 nva-dmz-1; do
  log ""
  log "  ---- $NVA_NAME ----"

  log "  [4a] iptables POSTROUTING MASQUERADE hit counters:"
  IPT=$(az vm run-command invoke \
    -g "$RG" -n "$NVA_NAME" \
    --command-id RunShellScript \
    --scripts 'sudo iptables -t nat -L POSTROUTING -v -n 2>&1' \
    --query 'value[0].message' -o tsv 2>/dev/null || echo "ERROR")
  log "$IPT"
  if echo "$IPT" | grep -qi "MASQUERADE"; then
    check_pass "$NVA_NAME: iptables MASQUERADE rule present in POSTROUTING"
  elif [[ "$IPT" == "ERROR" ]]; then
    check_warn "$NVA_NAME: iptables query failed"
  else
    check_fail "$NVA_NAME: MASQUERADE rule not found — NVA cloud-init may not have completed"
  fi

  log ""
  log "  [4b] Connection tracking (conntrack -L | head; fallback: ss -s):"
  CTBL=$(az vm run-command invoke \
    -g "$RG" -n "$NVA_NAME" \
    --command-id RunShellScript \
...
# Concurrent: fire spoke curl in background while running tcpdump on nva-dmz-0
log ""
...
if [[ "$TCPD" != "ERROR" ]]; then
  check_pass "tcpdump on nva-dmz-0 succeeded (NVA is forwarding packets)"
else
  check_warn "tcpdump on nva-dmz-0 failed (non-fatal)"
fi
...
if [[ $FAIL -gt 0 ]]; then
  ...
  log "    - NVA cloud-init not finished (check: az vm boot-diagnostics get-boot-log)"
...
log "  For deeper analysis, run: ./scripts/enable-monitoring.sh"
