...
Note the variables used does not work over Azure CLI inside PowerShell.

### Deploy this solution

All the content of this lab has been also available in above .azcli that you can rename as .sh (shell script) and execute them. Although the recommendation is you run step-by-step to get familiar with the provisioning process and also have opportunity to test connectivity on all ends (intra-region, inter-region and branches).
