...
// Standard Public Load Balancer for DMZ PA firewalls
//
// Frontend: single Standard static public IP (pip-lb-public)
// Backend:  PA untrust NICs (joined from palo-alto.bicep)
// Rules:
//   - LB rule: TCP 22 inbound → backend 22 (SSH; disableOutboundSnat=true)
//   - Outbound rule: All protocols SNAT via public IP (egress for PA untrust)
// Health probe: TCP 22
// =============================================================================
...
resource lb 'Microsoft.Network/loadBalancers@2023-11-01' = {
  ...
  properties: {
    ...
    loadBalancingRules: [
      {
        // Inbound SSH to backend; disableOutboundSnat avoids conflict with outbound rule
        name: 'ssh-inbound'
        properties: {
    ...
    outboundRules: [
      {
        // SNAT all PA untrust outbound traffic through the public IP
        name: 'snat-outbound'
        properties: {
          frontendIPConfigurations: [
            {
...
@description('Backend pool resource ID (used by PA untrust NICs)')
output backendPoolId string = lb.properties.backendAddressPools[0].id

@description('Public IP address of the LB frontend (SNAT egress / SSH management IP)')
