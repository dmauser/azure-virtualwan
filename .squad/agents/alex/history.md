# Alex — History

**Last Updated:** 2026-07-28  
**Note:** Full session history archived to history-archive.md (19.6 KB).

## Core Identity

- **Role:** Network Engineer  
- **Stack:** Azure CLI, Bicep, ARM JSON, Bash/PowerShell
- **Domain:** Azure Networking (Virtual WAN, custom NVAs, Routing Intent, ER/VPN)
- **Key Labs Authored:** svh-dynamic-er-ri, nva-spoke-internet

---

## Key Learnings (Session: nva-spoke-internet)

### Virtual WAN Custom NVA Routing (no Routing Intent)

Two CLI operations required for spoke→NVA→internet traffic:

1. **DMZ connection static route** — az network vhub connection create with --route-name, --address-prefixes "0.0.0.0/0", --next-hop <ILB-IP>
2. **Hub defaultRouteTable static route** — az network vhub route-table route add --destination-type CIDR --destinations "0.0.0.0/0" --next-hop-type ResourceID --next-hop $CONN_DMZ_ID

**Key distinction:** Do NOT set --internet-security true when using custom static routes (that flag is only for Routing Intent modes).

### VNet Connections Must Be Post-Deploy

VNet connections cannot be created in Bicep due to hub routingState timing issues. Always poll routingState == Provisioned before creating connections.

### On-Prem BGP-over-IPsec

- **strongSwan**: IKEv2, one conn per hub GW instance (active/active=2), route-based (0.0.0.0/0), auto=start, dpdaction=restart
- **FRR**: router bgp 65001, two neighbors, ebgp-multihop 5 required (BGP peer not directly connected)
- **PSK pattern**: Create connection first, then vpn-site-link-conn sharedkey update separately
- **Hub queries**: ipConfigurations[0].publicIpAddress and bgpSettings.bgpPeeringAddresses[0].defaultBgpIpAddresses[0]

### Bicep Output Contract (nva-spoke-internet)

Consumed outputs: location, vwanName, hubName, hubId, dmzVnetId, spoke1VnetId, spoke2VnetId, ilbFrontendIp, publicLbPublicIp, nvaNames, vpnGatewayName, onpremVnetId, onpremNvaPublicIp, onpremNvaPrivateIp, onpremNvaName, onpremVmName

Params TO Bicep: location, adminUsername, adminPassword, vmSize, deployOnPrem, onpremBgpAsn

Key constant: ILB frontend IP = **10.0.0.68** (in DMZ 10.0.0.0/24)

### Deploy.ps1 Fixes (D1–D4)

1. Non-interactive password via stdin
2. DeployOnPrem prompt fix using System.Management.Automation.PSBoundParametersDictionary.ContainsKey()
3. DefaultRtId reuse (eliminate redundant az account show)
4. Windows empty-string bug: pre-create NIC instead of passing --public-ip-address ""

### Live Validation Results

**Deployment on DMAUSER-FDPO (2026-07-24, Phases 1–13):**
- **Resources:** All 24 present (hub, vWAN, 3 VNets, 2 NVAs, 2 workload VMs, Public LB, ILB, etc.)
- **Hub routingState:** Provisioned ✅
- **Routing:** defaultRouteTable 0/0→conn-dmz ✅ | conn-dmz static 0/0→10.0.0.68 ✅
- **Effective routes:** Spoke NICs see 0.0.0.0/0 via VirtualNetworkGateway ✅
- **Egress (curl ifconfig.io):** Returned Public LB PIP (20.65.77.169) ✅
- **East-west (Spoke1→Spoke2):** 3/3 pings, 0% loss ✅
- **LB metrics:** All 7 metrics on public LB + 3 on ILB collected ✅

**Status: PASS 12 / FAIL 0 / WARN 2** (warnings = Network Watcher not enabled, expected)

---

## 2026-07-27 — PA Lab Sessions (Diagram, ILB Fix, Bootstrap, Dual-VR)

**PA Diagram:** Rebuilt nva-spoke-internet/media/nva-spoke-internet.svg (18 KB, 1400×800) with PA-specific NIC callouts and LBs. Excalidraw v2 (117 elements, 106 KB) matching SVG layout. Python workaround for hex colors in PowerShell.

**ILB Health-Probe Symmetry Bug & Fix:** Azure LB probes from 168.63.129.16; without /32 static route, probe SYN-ACK exits wrong NIC → Azure drops as spoof → 0% healthy. Fix: Static route azure-probe-via-trust (168.63.129.16/32 → ethernet1/2) forces replies out the interface probes arrived on. Pattern: **any NVA behind Azure ILB MUST have /32 host-route to 168.63.129.16**. Unrelated to security policy or NAT; purely routing.

**PAN-OS Bootstrap:** init-cfg.txt with type=dhcp-client, op-command-modes=mgmt-interface-swap (REQUIRED for 3-NIC Azure). bootstrap.xml: Interface-management-profiles under /network/, zones under vsys/zone, non-syn-tcp=yes for HA-ports ILB. LB probes TCP/22 via interface-management-profile (no SSH server needed). BYOL eval: 30-day full-feature period; routing, NAT, security policy all work. NAT: dynamic-ip-and-port source-translation (ethernet1/1) → double-SNAT (trust NIC IP → Public LB PIP).

**Dual-VR Canonical Fix:** Single-VR + /32 host-route is anti-pattern when BOTH ELB and ILB present. Solution: Two virtual routers — VR-Untrust (eth1/1, 0/0→10.0.0.33 for ELB probe symmetry, internet egress) and VR-Trust (eth1/2, 168.63.129.16/32 for ILB, 10/8 for spokes, 0/0→next-vr VR-Untrust). Inter-VR via <next-vr> nexthop. Zones unchanged; security policy evaluated at trust→untrust. LB probes = self-traffic (interface-management-profile), no policy rules needed.

**MG-policy Bootstrap Blocker + Post-Boot API Fallback:** Policy allowSharedKeyAccess=false blocks az storage account keys list (shared-key SMB auth for Azure Files bootstrap) → firewalls boot factory-default, no config → spoke egress fails. Root cause: config-delivery only, NOT routing. Fallback fix (Phase 7b): Post-boot XML API (keygen → import bootstrap.xml → load → commit → poll) applies config via management IP. Idempotent, self-signed cert workarounds (curl -sk, PS TrustAll/SkipCertificateCheck).

**PA lab review gate:** PASS (Amos). Live deploy ready (opt-in).

---

## 2026-07-28 — PALO-ALTO-CONFIG.md Canonical Reference

**Status:** Implemented & merged to main (commit d5e242a)

Coordinator authored 
va-spoke-internet-paloalto/PALO-ALTO-CONFIG.md (14,168 chars), authoritative over README's single-VR ASCII diagram. Contents: Azure Standard LB config (ILB HA-ports, Public SNAT, TCP/22 probes from 168.63.129.16), PAN-OS day-0 bootstrap (dual-VR design, 168.63.129.16/32 route, NAT, security policy), end-to-end egress walk, verification commands, Azure Learn citations. Key decisions: dual-VR pattern (canonical fix for Active-Active + ELB/ILB), HA-ports floating IP, double-SNAT, Bootstrap API fallback Phase 7b. Living reference for all future Palo Alto Azure deployments.
