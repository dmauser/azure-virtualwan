...
**2026-07-27 (bootstrap-fallback session):** DMAUSER-FDPO live-deployment debug & docs update.
- Live deploy to DMAUSER-FDPO: 15/17 validation PASS, 2 spoke→Internet egress FAIL. Root cause identified: management-group policy `allowSharedKeyAccess=false` blocks PAN-OS Azure Files bootstrap (shared-key SMB required; OAuth unsupported for Files data plane). Firewalls booted factory-default, no security policy or NAT rules.
- **Corrected prior misconception:** This is NOT a routing/network-design defect. The hub routing topology (0.0.0.0/0 → conn-dmz → ILB 10.0.0.68 HA-ports) is CORRECT and validated by the Linux twin (nva-spoke-internet/) passing identical live deployment. The failure was config-delivery only.
- **The fix (Naomi + Alex, already committed):** Phase 5b (`deploy.ps1`/`deploy.sh`) detects policy-blocked shared-key access, logs a warning, skips doomed SMB uploads. Phase 7b (post-VM-boot fallback) automatically invokes `apply-panos-config.ps1`/`.sh`, which polls each PA management IP (via `pip-pa-0-mgmt`/`pip-pa-1-mgmt`, 10–15 min wait for API readiness) and applies `bootstrap.xml` via PAN-OS XML API (import configuration → load → commit), idempotent.
- Documented the bootstrap-blocker + Phase 7b fallback in README.md "Known Limitations & Bootstrap Fallback" section and expanded EXPECTED-RESULTS.md "Residual Risks" table. Emphasized that network routing is correct; problem was config delivery.
- Learnings: Always verify whether a subscription enforces restrictive storage-account policies before live deploy. Azure Files bootstrap can silently fail when `allowSharedKeyAccess=false`, creating the illusion of a network/routing bug until you trace into the PA config log. The API fallback is subscription-portable and already built-in.
